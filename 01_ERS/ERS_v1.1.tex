...
\begin{document}
	...
	\begin{titlepage}
		...
		{\normalsize\bfseries Versión del documento:} 1.1 \quad (Línea base: \texttt{baseline-v1.1})\\[4pt]
		\textbf{Repositorio GitHub:}\\
		\url{https://github.com/MiloSaurio4kHD/ISR401-PFC-ERS-EQUIPO-E.git}
		
		\textbf{Ultimo commit a considerar:}\\
		\normalsize Domingo 9 de Agosto del 2026
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{@{}l l l X@{}}
			...
			1.1 & 09/08/2026 & Equipo MundiPets & PE4 -- Unidad IV: incorpora las 23 correcciones derivadas de la inspección Fagan (D-01 a D-23) y de las solicitudes de cambio RFC-01, RFC-02 y RFC-03 aprobadas por el CCB. Corrige la cifra de RNF en la visión general (1.5), precisa el alcance de la validación veterinaria (RF-09, RD-03), el mecanismo de verificación de identidad (RF-12), los siete factores de evaluación de compatibilidad (RF-06), el reporte manual y la moderación de mensajes (RF-07, RD-01), los criterios de rechazo de imágenes (RF-17), la trazabilidad de lote en vacunación (RF-24), la protección de contacto y ubicación en el aviso de extravío (RF-25, en cumplimiento de RNF-16), cuatro enlaces de la matriz de trazabilidad (TR-10, TR-11, TR-33, TR-38) y seis requisitos no funcionales de verificabilidad y factibilidad (RNF-03, RNF-06, RNF-07, RNF-11, RNF-12, RNF-13, RNF-14). Línea base establecida mediante el tag anotado \texttt{baseline-v1.1}. \\
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	externas, los 25 requisitos funcionales, los 16 requisitos no funcionales, los
	...
	\begin{atributos}{RF-06}{RF-06 --- Evaluar compatibilidad entre mascotas para cruza.}
		...
		Prioridad (MoSCoW) & Must \\
		Criterio de verificación & Seleccionar dos mascotas y comprobar que la evaluación considera raza, edad, estado de salud, antecedentes genéticos, parentesco, comportamiento e historial médico; verificar además la información comparativa y la advertencia de resultado orientativo. \\
	\end{atributos}
	...
	\begin{atributos}{RF-07}{RF-07 --- Sistema de mensajería entre usuarios.}
		...
		Descripción & El sistema deberá permitir la comunicación entre propietarios e interesados mediante mensajes para coordinar procesos de adopción y cruza, y deberá permitir al usuario reportar mensajes o interacciones que considere spam, lenguaje ofensivo o acuerdos externos, registrando el motivo del reporte asociado a la conversación. \\
		Actor & Propietario de mascota, Adoptante, Interesado en cruza \\
		...
		Postcondiciones & El mensaje queda almacenado y disponible para ambos participantes, tras haber sido procesado por el componente de moderación automática conforme a RNF-15; si se marca como inapropiado, el envío se bloquea y se muestra la categoría detectada. \\
		Prioridad (MoSCoW) & Could \\
		Criterio de verificación & Enviar un mensaje entre dos usuarios y verificar que ambos puedan visualizarlo; enviar un mensaje inapropiado y comprobar que el sistema lo bloquea mostrando la categoría detectada. \\
	\end{atributos}
	...
	\begin{atributos}{RF-09}{RF-09 --- Validar la información médica de una mascota.}
		...
		Descripción & El sistema deberá permitir que una veterinaria autorizada realice una validación referencial de la información médica registrada de una mascota, sin que dicha validación constituya certificación clínica ni reemplace una consulta veterinaria presencial. \\
		Actor & Veterinaria \\
	...
	\begin{atributos}{RF-12}{RF-12 --- Verificar la identidad de los usuarios.}
		...
		Descripción & El sistema deberá verificar la identidad del usuario mediante sus datos básicos y un documento nacional de identidad legible, antes de habilitar procesos de adopción o cruza. La verificación será rechazada cuando la documentación sea ilegible o incompleta. \\
		Actor & Propietario de mascota, Adoptante, Interesado en cruza \\
		...
		Prioridad (MoSCoW) & Must \\
		Criterio de verificación & Registrar un usuario con un documento nacional de identidad legible y verificar que el sistema indique el estado de verificación; repetir con un documento ilegible o incompleto y comprobar el rechazo. \\
	\end{atributos}
	...
	\begin{atributos}{RF-17}{RF-17 --- Validar imágenes.}
		...
		Descripción & El sistema deberá utilizar un componente de inteligencia artificial para validar las imágenes cargadas por los usuarios, verificando que correspondan al tipo de fotografía esperado y detectando, como mínimo, contenido inapropiado, imagen borrosa o tipo de fotografía incorrecto; cuando una imagen sea rechazada, se mostrará el motivo conforme a RNF-14. \\
		Actor & Propietario de mascota, Veterinaria \\
	...
	\begin{atributos}{RF-24}{RF-24 --- Registrar trazabilidad de cepa, lote y aplicador de cada vacuna.}
		...
		Descripción & El sistema deberá permitir registrar, además de la fecha, la cepa o marca del biológico, el número de lote y el profesional que aplicó cada vacuna dentro del carnet de vacunación. \\
		Actor & Veterinaria, Propietario de mascota \\
		Origen (ID de evidencia) & EV-15, EV-16 \\
		Entradas & Cepa/marca del biológico, número de lote, nombre del aplicador, fecha de aplicación. \\
		...
		Prioridad (MoSCoW) & Should \\
		Criterio de verificación & Registrar una vacuna con cepa/marca, lote y aplicador, y comprobar que los tres datos de trazabilidad aparecen en el carnet. \\
	\end{atributos}
	...
	\begin{atributos}{RF-25}{RF-25 --- Publicar aviso de mascota extraviada.}
		...
		Descripción & El sistema deberá permitir al propietario publicar un aviso público de mascota extraviada, incluyendo fotografías e identificador de microchip. Los datos de contacto directo permanecerán ocultos por defecto y solo se mostrarán cuando el propietario autorice expresamente su visibilidad mediante RF-11. \\
		Actor & Propietario de mascota \\
		Origen (ID de evidencia) & EV-12, EV-16 \\
		Entradas & Fotografías, zona aproximada de última ubicación conocida (radio $\geq$ 1 km), número de microchip, datos de contacto. \\
		...
		Prioridad (MoSCoW) & Could \\
		Criterio de verificación & Publicar un aviso de mascota extraviada y comprobar, desde una cuenta sin autorización, que no aparecen la dirección exacta ni los datos de contacto directo; verificar que se muestra la zona aproximada. \\
	\end{atributos}
	...
	\begin{atributos}{RNF-03}{RNF-03 --- Tiempo de respuesta del sistema.}
		...
		Prioridad (MoSCoW) & Should \\
		Criterio de verificación & Ejecutar una prueba de carga con la herramienta k6 sobre el entorno de staging (2 vCPU / 4 GB RAM) con 100 usuarios concurrentes y comprobar que el percentil 95 del tiempo de respuesta no supera el valor objetivo. \\
	\end{atributos}
	...
	\begin{atributos}{RNF-06}{RNF-06 --- Exactitud del componente de inteligencia artificial.}
		...
		Métrica & Porcentaje de recomendaciones consideradas adecuadas por un médico veterinario durante las pruebas, sobre un conjunto de casos previamente evaluado y documentado por veterinarios colaboradores del proyecto. \\
		Valor objetivo & $\geq$ 85\,\% de coincidencia con la evaluación veterinaria. \\
		Prioridad (MoSCoW) & Must \\
		Criterio de verificación & Comparar las recomendaciones emitidas por la IA con las evaluaciones de veterinarios sobre un conjunto de al menos 100 casos de prueba, estratificado por especie, raza y rango etario. \\
	\end{atributos}
	...
	\begin{atributos}{RNF-07}{RNF-07 --- Precisión en la validación de imágenes.}
		...
		Característica ISO 25010 & Adecuación funcional \\
		Descripción & El componente de inteligencia artificial deberá clasificar correctamente al menos el 95\,\% de las imágenes cargadas como válidas (fotografía nítida de la mascota o del documento solicitado) o inválidas (imagen borrosa, contenido no relacionado o tipo de fotografía incorrecto). \\
	...
	\begin{atributos}{RNF-11}{RNF-11 --- Portabilidad entre navegadores y dispositivos.}
		...
		Descripción & La plataforma web deberá funcionar correctamente en Chrome y Firefox (últimas dos versiones estables a la fecha de prueba) y Safari (última versión estable de iOS/macOS), y adaptarse de forma responsiva a resoluciones de 360×640 px (móvil) y 1366×768 px (escritorio) como referencia mínima. \\
	...
	\begin{atributos}{RNF-12}{RNF-12 --- Escalabilidad ante crecimiento de usuarios.}
		...
		Descripción & El sistema deberá soportar el incremento de usuarios concurrentes permitiendo escalar horizontalmente los servidores de aplicación, de modo que el tiempo de respuesta P95 no aumente más del 50\,\% respecto del valor objetivo definido en RNF-03 al triplicar la carga nominal. \\
	...
	\begin{atributos}{RNF-13}{RNF-13 --- Explicabilidad de la recomendación de compatibilidad para cruza (RF-06).}
		...
		Prioridad (MoSCoW) & Must \\
		Criterio de verificación & Solicitar una evaluación de compatibilidad, comprobar tiempo/extensión de la explicación y aplicar una encuesta de comprensión a usuarios técnicos y no técnicos ($\geq$ 4/5 de calificación media); verificar en la matriz de trazabilidad que la evidencia de la explicación, la medición de tiempo y la encuesta de comprensión están enlazadas explícitamente a RNF-13. \\
	\end{atributos}
	...
	\begin{atributos}{RNF-14}{RNF-14 --- Explicabilidad del rechazo o aceptación de imágenes (RF-17).}
		...
		Prioridad (MoSCoW) & Should \\
		Criterio de verificación & Cargar imágenes inválidas, comprobar que cada rechazo muestra un motivo específico, y encuestar la comprensión del motivo mostrado; verificar en la matriz de trazabilidad la correspondencia entre RNF-14, RF-17, los criterios de aceptación y el caso de prueba de la explicación. \\
	\end{atributos}
	...
	\begin{atributos}{RD-01}{RD-01 --- Integración de componentes de inteligencia artificial.}
		...
		Descripción & El sistema deberá incorporar componentes de inteligencia artificial para la evaluación de compatibilidad entre mascotas, la validación de imágenes cargadas por los usuarios y la moderación automática de mensajes, manteniendo los tres subcomponentes definidos en la arquitectura. \\
	...
	\begin{atributos}{RD-03}{RD-03 --- Validación profesional de la información médica.}
		...
		Descripción & El sistema deberá permitir que la validación referencial de la información médica sea realizada únicamente por veterinarias autorizadas, sin que dicha validación equivalga a una certificación clínica. \\
	...
	\begin{landscape}
		\begin{longtable}{@{}p{0.8cm} p{1.3cm} p{1.5cm} p{2.2cm} p{1.5cm} p{1.8cm} p{1.6cm} p{1.3cm} p{1.3cm} p{1.3cm} p{1.5cm}@{}}
			...
			TR-10 & --- & --- & Controles preventivos & Propietario & EV-02, EV-04, EV-06 & RF-10 & \emph{Sin CU específico (D-12)} & --- & --- & --- \\ \midrule
			
			TR-11 & LOPDP & Art. 12, 13 y 10 lit. e) & Administrar privacidad & Propietario & EV-02, EV-05, EV-06, EV-07, EV-08 & RF-11 & CU-12 & HU-08 & CA-08 & MU-08 \\ \midrule
			...
			TR-33 & --- & --- & Encuentros de socialización supervisados & Propietario & EV-13 & RF-20 & \emph{Sin CU específico (D-14)} & --- & --- & --- \\ \midrule
			...
			TR-38 & --- & --- & Aviso de mascota extraviada & Propietario & EV-12, EV-16 & RF-25 & \emph{Sin CU específico (D-15)} & --- & --- & --- \\ \midrule
